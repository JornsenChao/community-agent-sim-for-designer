...
现在代码里有哪些冗余的地方？请你全部指出，包括：哪个文件，哪个功能，哪行。

## 想法拓展

由于可行性的考虑，我有另一个想法：不再是”让设计师上传 城市大型公园 或 城市规划设计 的设计草图，计算机视觉识别，结合 OSM，创建一个适合 agent 模拟的空间环境，然后让 agent 在其中模拟他们的一天。“，而是继续利用现在的代码，将目标调整为 ”城市设计师在地块 zoning 阶段，通过 各种 agent 的模拟，来获得可能的社区意见（避免完全的上帝视角和专家视角）“。另外，这个新的想法会通过模块化，来帮助在不同阶段/颗粒度的任务；比如，地块的 zoning 可以替换为 ”一条街上各类商铺的分配“”一个城市片区各类 amenity 的布局“等，可以服务建筑规划下不同的场景。
我之所以会有上述想法，是因为不确定”用户上传设计草图“和”cv 技术识别草图“，”草图融合 osm 空间数据“这些内容有多少不确定性，而且草图可能会很复杂，要能成功识别，对草图质量和用户花时间标注的要求有点高。

我想请你评估上面提到的想法，和我之前的想法 。从可行性，学术创新性，实践中对设计师的帮助，以及其他你觉得有必要的维度评价。

基于我现在的 【城市道路与建筑物信息】和 【有个人背景，出发点 home，目的地 work】的 agent，你觉得可以怎么构建下面几个技术：

1. 将用户的输入信息覆盖原有 OSM 或者增加到原有 OSM:
   1.1 比如”将某个已建成地块从 功能 A 改成 功能 B“ 应用在 OSM 建筑道路信息上
   1.2 不覆盖原有 amenity，用户实验”给这一块地方增加某些 amenity“
   1.3 某一篇街区的 POI 特别是零售等商业 的可能性。可以覆盖现有 POI 或 创建新的 POI
2. 上述结合后的空间数据，你需要找到一个适合 你推荐的 MESA ABM 框架 的空间环境，来让 agent 在其中移动？
3. agent 在 环境 （osm 获得的 ，之后被用户的用地功能修改缩覆盖）中，agent 的内在属性（比如说 BDI/日常行为）会怎么影响对于当前 时间+ 空间环境 的判断，从而影响他们的行为？先不考虑 agent 之间的互动。
4. agent 在模拟结束后，是否能记录 abm，包括当天的所有活动，以及 agent 的所有想法？这样模拟结束后，用户对 agent 提问，可以得到更多 insight
5. 播放所有 Agent 移动轨迹，包括他们停在哪里，当时有什么想法

## 论文启发

请你搜索这两篇关于 agent 的论文： 《Large Language Models as Urban Residents》和《Generative Agents: Interactive Simulacra of Human Behavior》
这两篇文字对我的项目可以有什么启发呢？
上面两篇论文对我的启发，可以怎么优化上面提到的这些点？
